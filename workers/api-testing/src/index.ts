import { ScanModel, TargetModel, EndpointModel, FindingModel, AuthProfileModel } from '@securityscan/database';
...
const logger = createLogger('worker-api-testing');

export async function runApiTestingWorker(scanId: string): Promise<number> {
  ...
  const apiEngine = new DetectionEngine();
  apiEngine.registerAll(OAuthSecurityChecks.getChecks());
  apiEngine.registerAll(BolaChecks.getChecks());
  apiEngine.registerAll(BflaChecks.getChecks());
  apiEngine.registerAll(MassAssignmentChecks.getChecks());

  const authProfiles = await AuthProfileModel.find({ projectId: scan.projectId });
  for (const profile of authProfiles) {
    if (profile.type === 'BEARER_TOKEN' || profile.type === 'JWT') {
      try {
        const parsed = JSON.parse(profile.encryptedConfiguration);
        if (parsed.token) {
          apiEngine.registerAll(JwtSecurityChecks.getChecks(parsed.token));
        }
      } catch {
        // Safe continue
      }
  ...
  for (const ep of endpoints) {
    ...
    try {
      const results = await apiEngine.runAll(ctx);
      ...
      logger.debug({ endpoint: ep.url, error: (err as Error).message }, 'API worker check error');
    }
  }
